...
int main()
{
  ofstream ofs1("../plob2_data/lyapunov_10_points.csv");
  ofstream ofs2("../plob2_data/diff_distance.csv");

  vector<double> X_true(N, F);
  vector<vector<double>> d(10, vector<double>(N, 0));
  vector<double> d_sum(10, 0.0), d0(10, 0.0), d_mse(10, 0.0);
  double d_sum_avg = 0.0, d_mse_avg = 0.0;
  ...
  for (int j = 1; j < time_steps; j++)
  {
    ...
    ofs1 << d_sum_avg / 10 / dt / j << endl;
    ofs2 << d_mse_avg / 10 << endl;
  }
